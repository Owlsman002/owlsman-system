...
with bk_col1:
    bk_home = st.number_input(f"{name_a} Win Odd", min_value=1.01, value=3.40, step=0.01, key="bk_home")
with bk_col2:
    bk_draw = st.number_input("Draw Odd", min_value=1.01, value=3.40, step=0.01, key="bk_draw")
with bk_col3:
    bk_away = st.number_input(f"{name_b} Win Odd", min_value=1.01, value=2.10, step=0.01, key="bk_away")

st.divider()
run_clicked = st.button("RUN ENGINE", type="primary", use_container_width=True)

if run_clicked:
    # Basic validation -- every stat must be a positive number, since several formula
    # steps divide by these values.
    inputs = {
        "Team A Goals Scored": ga, "Team A Goals Conceded": ca,
        "Team A Shots on Target For": sota, "Team A Shots on Target Against": sotaga,
        "Team A Big Chances": bca, "Team A xG": xga,
        "Team B Goals Scored": gb, "Team B Goals Conceded": cb,
        "Team B Shots on Target For": sotb, "Team B Shots on Target Against": sotbga,
        "Team B Big Chances": bcb, "Team B xG": xgb,
    }
    zero_fields = [k for k, v in inputs.items() if v <= 0]

    if zero_fields:
        st.error("Every stat field must be greater than 0. Check: " + ", ".join(zero_fields))
    else:
        team_a = TeamStats(name=name_a or "Team A", goals_scored=ga, goals_conceded=ca,
                            shots_on_target_for=sota, shots_on_target_against=sotaga,
                            big_chances=bca, expected_goals=xga)
        team_b = TeamStats(name=name_b or "Team B", goals_scored=gb, goals_conceded=cb,
                            shots_on_target_for=sotb, shots_on_target_against=sotbga,
                            big_chances=bcb, expected_goals=xgb)
        ...
        st.table({
            "Outcome": [f"{team_a.name} Win", "Draw", f"{team_b.name} Win"],
            "Baseline Odds": [result.baseline_odds["oh"], result.baseline_odds["od"], result.baseline_odds["oa"]],
            "Master Odds": [result.master_odds["oh"], result.master_odds["od"], result.master_odds["oa"]],
            "Bookmaker Odds": [bk_home, bk_draw, bk_away],
        })

        st.subheader("Separate Dominance Index")
        d1, d2 = st.columns(2)
        d1.metric(team_a.name, result.dominance_home,
                   delta="Resilient" if result.dominance_home >= 0 else "Fragile")
        d2.metric(team_b.name, result.dominance_away,
                   delta="Resilient" if result.dominance_away >= 0 else "Fragile")
        ...
        st.write(f"Draw difference (Bookmaker − Consensus) = **{result.draw_diff_pct} pts**")

        st.subheader("Value Margin vs Bookmaker")
        st.table({
            "Outcome": [f"{team_a.name}", "Draw", f"{team_b.name}"],
            "Value Margin %": [result.value_margin["h"], result.value_margin["d"], result.value_margin["a"]],
        })
        ...
        for col, (h, a, p) in zip((sc1, sc2, sc3), result.top_scores):
            col.metric(f"{h} - {a}", f"{round(p * 100, 2)}%")
        ...
        with st.expander("Show raw EngineResult (debug)"):
            st.json({
                ...
                "model_consensus_draw_pct": result.model_consensus_draw_pct,
                "draw_diff_pct": result.draw_diff_pct,
            })
else:
    st.info("Fill in both teams' stats and the bookmaker odds above, then click **RUN ENGINE**.")
